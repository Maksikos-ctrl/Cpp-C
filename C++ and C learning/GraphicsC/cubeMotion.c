...
GLfloat light_diffuse[]  = {1.0f, 0.5f, 0.31f};  /* Red diffuse light. */ 
...
void timer(int val) {
  ...
  float delta = (currTime - prevTime) / (float) CLOCKS_PER_SEC;
  prevTime = currTime;
  ...
  // Call the timer callback again after 10 milliseconds
...
{  
   ...
   glutCreateWindow("3D Cube Motion");  
